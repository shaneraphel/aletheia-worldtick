...
def read_opendrive_bipartite(path, n_left):
    if n_left < 1:
        raise ValueError("uncompiled OpenDRIVE network is absence")
    bip = []
    for a, b in read_opendrive_edges(path):
        if a >= n_left or b < n_left:
            raise ValueError("uncompiled OpenDRIVE network is absence")
        bip.append((a, b - n_left))
    n_right = 1 + max(v for _, v in bip)
    return n_left, n_right, bip
