...
def main():
    ...
    try:
        ...
        empty = "raised"
    rec = {
        "schema": "bridgelock.show_opendrive.v1",
        "used": "https://www.asam.net/standards/detail/opendrive/",
        "built": "triangle-plus-pending OpenDRIVE, bridge 1; empty network is absence",
        "ours": {"n_roads": opendrive_occupancy(XODR), "bridges": n_bridges(len(graph), edges), "empty": empty},
        "python": sys.version.split()[0],
        "platform": platform.platform(),
        "n_parameters": 0,
        "gradient_descent_steps": 0,
    }
    if rec["ours"]["bridges"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bridgelock opendrive show identity failed")
