...
EEG = Path(__file__).resolve().parent / "resources" / "synthetic" / "sub-01_task-spike_nihon.EEG"
...
def theirs() -> dict:
    ...
    try:
        ...
        empty = "raised"
    nolog = Path("/tmp/aletheia-nolog-nihon")
...
def main():
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours.EEG")
    p.write_bytes(b"")
    try:
        read_nihon(p)
        empty = "accepted"
    ...
    empty_log = "raised"
    try:
        write_nihon(Path("/tmp/aletheia-empty-nihon-log"), [1], events=[])
        empty_log = "accepted"
    except ValueError:
        empty_log = "raised"
    rec = {
        "schema": "spikelock.show_nihon.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads 8 Nihon Kohden samples on C3 and go/end at 1.0/2.0 s; empty Nihon Kohden is absence",
        "theirs": theirs(),
        "ours": {
            "n": nihon_occupancy(EEG),
            "samples": read_nihon(EEG)["samples"],
            "empty": empty,
            "empty_log": empty_log,
    ...
    if rec["ours"]["n"] != 8 or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock nihon show identity failed")
    if rec["ours"]["empty_log"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("spikelock nihon log identity failed")
    if rec["theirs"]["n_times"] != 8 or rec["theirs"]["empty_eeg"] != "raised":
