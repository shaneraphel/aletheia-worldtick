...
DATA = Path(__file__).resolve().parent / "resources" / "synthetic" / "sub-01_task-spike_nicolet.data"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty.data")
    p.write_bytes(b"")
    try:
...
def main():
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours.data")
    p.write_bytes(b"")
    try:
        read_nicolet(p)
        empty = "accepted"
    ...
    try:
        write_nicolet(Path("/tmp/aletheia-empty-nicolet-samples"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "spikelock.show_nicolet.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads 8 Nicolet samples on Cz; empty Nicolet is absence",
        "theirs": theirs(),
        "ours": {
            "n": nicolet_occupancy(DATA),
            "samples": read_nicolet(DATA)["samples"],
            "empty": empty,
    ...
    if rec["ours"]["n"] != 8 or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock nicolet show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("spikelock nicolet empty identity failed")
    if rec["theirs"]["n_times"] != 8 or rec["theirs"]["empty_nicolet"] != "raised":
