...
NXE = Path(__file__).resolve().parent / "resources" / "synthetic" / "sub-01_task-spike.nxe"
...
def main():
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours.nxe")
    p.write_bytes(b"")
    try:
        read_eximia(p)
        empty = "accepted"
    ...
    try:
        write_eximia(Path("/tmp/aletheia-empty-eximia-samples.nxe"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "spikelock.show_eximia.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads 8 Eximia samples on Cz; empty Eximia is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["n"] != 8 or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock eximia show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("spikelock eximia empty identity failed")
    if rec["theirs"]["n_times"] != 8 or rec["theirs"]["empty_nxe"] != "raised":
