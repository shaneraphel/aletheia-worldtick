...
def main():
    samples = read_eeglab(SET)
    steps = [(int(v), 0) for v in samples]
    empty = "raised"
    p = Path("/tmp/aletheia-empty-read.set")
    ...
    empty_events = "raised"
    try:
        write_eeglab(Path("/tmp/aletheia-empty-events.set"), [1], events=[])
        empty_events = "accepted"
    except ValueError:
        empty_events = "raised"
    rec = {
        "schema": "spikelock.show_eeglab.v1",
        "used": "https://eeglab.org/",
        "built": "three EEGLAB points, occupancy 3; EEG.event go/end; empty set is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["nyquist"] != 3 or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock eeglab show identity failed")
    if rec["ours"]["empty_events"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("spikelock eeglab event identity failed")
