...
| **BrainVision** | OpenNeuro `.vhdr` / `.vmrk` / `.eeg`; Stimulus markers copy `trial_type` | 0 channels is absence; empty markers are absence |
...
| **GDF 1.25** | BioSig 1.x field-major header + int16 + mode-0 event byte | 0 channels is absence |

Playable files live in `resources/synthetic/`. The occupancy kernels are
`edfocc.py`, `bidseeg.py`, `xdfocc.py`, `bvocc.py`, `wfdbocc.py`, `eeglabocc.py`, `cntocc.py`, `persystocc.py`, `nsxocc.py`, `nicoletocc.py`, `nihonocc.py`, `eyelinkocc.py`, `eximiaocc.py`, `nirxocc.py`, `hitachiocc.py`, `snirfocc.py`, `boxyocc.py`, `curryocc.py`, `egiocc.py`, and `gdfocc.py`.
An empty tape raises.
