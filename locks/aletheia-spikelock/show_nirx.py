...
NIRX = Path(__file__).resolve().parent / "resources" / "synthetic" / "sub-01_task-spike_nirx"
LOCK = "spikelock"
N = 8
...
def main():
    ...
    try:
        ...
        empty_evt = "raised"
    rec = {
        "schema": "spikelock.show_nirx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads 8 NIRx samples on S1_D1 and stim 1.0/2.0 at 1.0/2.0 s; empty NIRx is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["n"] != N or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock nirx show identity failed")
    if rec["ours"]["empty_evt"] != "raised" or rec["ours"]["annot"] != ["1.0", "2.0"]:
        raise SystemExit("spikelock nirx evt identity failed")
