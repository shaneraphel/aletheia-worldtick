...
"""Show: MNE on a BrainVision tape (an empty header)."""
...
from bvocc import brainvision_occupancy, read_brainvision, read_brainvision_annotations

VHDR = Path(__file__).resolve().parent / "resources" / "synthetic" / "sub-01_task-spike.vhdr"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty.vhdr")
    p.write_text("")
    try:
...
def main():
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours.vhdr")
    p.write_text("")
    try:
        read_brainvision(p)
        empty = "accepted"
        ...
        empty = "raised"
    rec = {
        "schema": "spikelock.show_brainvision_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "MNE reads the BrainVision tape, 8 samples; empty .vhdr is absence",
        "theirs": theirs(),
        "ours": {
            "n": brainvision_occupancy(VHDR),
            "samples": read_brainvision(VHDR)["samples"],
            "empty": empty, "annot": read_brainvision_annotations(VHDR),
        },
    ...
    if rec["ours"]["n"] != 8 or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock brainvision show identity failed")
    if rec["theirs"]["n_times"] != 8 or rec["theirs"]["empty_vhdr"] != "raised":
        raise SystemExit("mne brainvision identity failed")
