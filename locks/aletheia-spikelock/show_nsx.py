...
NSX = Path(__file__).resolve().parent / "resources" / "synthetic" / "sub-01_task-spike.ns3"
...
def main():
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours.ns3")
    p.write_bytes(b"")
    try:
        read_nsx(p)
        empty = "accepted"
    ...
    try:
        write_nsx(Path("/tmp/aletheia-empty-nsx-samples.ns3"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "spikelock.show_nsx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads 8 NSX samples on Cz; empty NSX is absence",
        "theirs": theirs(),
        "ours": {
            "n": nsx_occupancy(NSX),
            "samples": read_nsx(NSX)["samples"],
            "empty": empty,
    ...
    if rec["ours"]["n"] != 8 or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock nsx show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("spikelock nsx empty identity failed")
    if rec["theirs"]["n_times"] != 8 or rec["theirs"]["empty_nsx"] != "raised":
