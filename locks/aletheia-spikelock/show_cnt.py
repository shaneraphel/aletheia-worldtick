...
CNT = Path(__file__).resolve().parent / "resources" / "synthetic" / "sub-01_task-spike.cnt"
...
def main():
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours.cnt")
    p.write_bytes(b"")
    try:
        read_cnt(p)
        empty = "accepted"
    ...
    empty_events = "raised"
    try:
        write_cnt(Path("/tmp/aletheia-empty-cnt-events.cnt"), [1], events=[])
        empty_events = "accepted"
    except ValueError:
        empty_events = "raised"
    rec = {
        "schema": "spikelock.show_cnt.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads 8 CNT samples and stim 1/2 at 1.0/2.0 s; empty CNT is absence",
        "theirs": theirs(),
        "ours": {
            "n": cnt_occupancy(CNT),
    ...
    if rec["ours"]["n"] != 8 or rec["ours"]["empty"] != "raised":
        raise SystemExit("spikelock cnt show identity failed")
    if rec["ours"]["empty_events"] != "raised" or rec["ours"]["annot"] != ["1", "2"]:
        raise SystemExit("spikelock cnt event identity failed")
    if rec["theirs"]["n_times"] != 8 or rec["theirs"]["empty_cnt"] != "raised":
