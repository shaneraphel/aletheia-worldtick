...
"""Show: I used MNE on a GDF 1.25 tape, and I refused an empty recording."""
...
from gdfocc import read_gdf, read_gdf_word
...
def theirs() -> dict:
    ...
    empty_annot = Path("/tmp/aletheia-empty-stem-annot.txt")
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem-mne.gdf")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty_annot = "raised"
    qa = Path("/tmp/aletheia-empty-ours-stem-annot.txt")
    qa.write_bytes(b"")
    try:
        ...
        empty_annot = "raised"
    rec = {
        "schema": "stemlock.show_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE annotations copy BIDS trial_type go/end; empty trial_type is absence; MNE reads GDF samples aba; suffix links 3; empty GDF is absence",
        "theirs": theirs(),
        "ours": {"annot": read_mne_annot_descriptions(ANNOT), "empty_annot": empty_annot, "edf_annot": read_edf_annotations(EDF), "word": word, "links": n_suffix_links(word)},
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba":
        raise SystemExit("stemlock mne show identity failed")
    if rec["theirs"]["gdf_n_times"] != 3 or rec["theirs"]["empty_gdf"] != "raised":
        raise SystemExit("mne stem identity failed")
