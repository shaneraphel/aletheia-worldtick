...
"""Show: edfio on an EDF tape (an empty recording)."""
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem-edfio.edf")
    p.write_bytes(b"")
    try:
        edfio.read_edf(p)
        empty = "accepted"
...
def main():
    word = "".join(chr(int(v)) for v in read_edf(EDF)["samples"])
    rec = {
        "schema": "stemlock.show_edfio.v1",
        "used": "https://github.com/the-siesta-group/edfio",
        "built": "edfio reads EDF samples aba; suffix links 3; empty EDF is absence",
        "theirs": theirs(),
        "ours": {"word": word, "links": n_suffix_links(word), "edf_annot": read_edf_annotations(EDF)},
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba":
        raise SystemExit("stemlock edfio show identity failed")
    if rec["theirs"]["n_samples"] != 3 or rec["theirs"]["empty"] != "raised":
        raise SystemExit("edfio stem identity failed")
