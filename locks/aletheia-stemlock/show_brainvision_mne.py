...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem.vhdr")
    p.write_text("")
    try:
...
def main():
    word = "".join(chr(int(v)) for v in read_brainvision(VHDR)["samples"])
    rec = {
        "schema": "stemlock.show_brainvision_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "MNE reads BrainVision aba; suffix links 3; empty .vhdr is absence",
        "theirs": theirs(),
        "ours": {"annot": read_brainvision_annotations(VHDR), "word": word, "links": n_suffix_links(word)},
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba":
        raise SystemExit("stemlock brainvision show identity failed")
    if rec["theirs"]["n_times"] != 3 or rec["theirs"]["empty_vhdr"] != "raised":
        raise SystemExit("mne stem brainvision identity failed")
