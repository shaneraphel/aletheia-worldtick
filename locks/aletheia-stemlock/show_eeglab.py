...
from eeglabocc import eeglab_occupancy, read_eeglab, read_eeglab_events, read_eeglab_word, write_eeglab
from ukkonen import n_suffix_links

SET = Path(__file__).resolve().parent / "resources" / "synthetic" / "tape.set"
...
def main():
    word = read_eeglab_word(SET)
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem.set")
    ...
    empty_events = "raised"
    try:
        write_eeglab(Path("/tmp/aletheia-empty-events-stem.set"), [1], events=[])
        empty_events = "accepted"
    except ValueError:
        empty_events = "raised"
    rec = {
        "schema": "stemlock.show_eeglab.v1",
        "used": "https://eeglab.org/",
        "built": "EEGLAB events aba have 3 suffix links; empty set is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba" or rec["ours"]["empty"] != "raised":
        raise SystemExit("stemlock eeglab show identity failed")
    if rec["ours"]["empty_events"] != "raised" or rec["ours"]["annot"] != ["a", "b", "a"]:
        raise SystemExit("stemlock eeglab event identity failed")
    if rec["theirs"]["annot"] != ["a", "b", "a"] or rec["theirs"]["onset"] != [0.0, 1.0, 2.0]:
