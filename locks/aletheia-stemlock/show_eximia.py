...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem.nxe")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-stem.nxe")
    p.write_bytes(b"")
    try:
        read_eximia(p)
        empty = "accepted"
    ...
    try:
        write_eximia(Path("/tmp/aletheia-empty-eximia-samples-stem.nxe"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "stemlock.show_eximia.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Eximia aba on Cz; suffix links 3; empty Eximia is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba" or rec["ours"]["empty"] != "raised":
        raise SystemExit("stemlock eximia show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("stemlock eximia empty identity failed")
