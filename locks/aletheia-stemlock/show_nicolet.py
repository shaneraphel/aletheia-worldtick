...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem.data")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-stem.data")
    p.write_bytes(b"")
    try:
        read_nicolet(p)
        empty = "accepted"
    ...
    try:
        write_nicolet(Path("/tmp/aletheia-empty-nicolet-samples-stem"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "stemlock.show_nicolet.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Nicolet aba; suffix links 3; empty Nicolet is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba" or rec["ours"]["empty"] != "raised":
        raise SystemExit("stemlock nicolet show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("stemlock nicolet empty identity failed")
