...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem.EEG")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    nolog = Path("/tmp/aletheia-nolog-nihon-stem")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-stem.EEG")
    p.write_bytes(b"")
    try:
        read_nihon(p)
        empty = "accepted"
    ...
    empty_log = "raised"
    try:
        write_nihon(Path("/tmp/aletheia-empty-nihon-log-stem"), [1], events=[])
        empty_log = "accepted"
    except ValueError:
        empty_log = "raised"
    rec = {
        "schema": "stemlock.show_nihon.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Nihon Kohden aba and go/end; suffix links 3; empty Nihon Kohden is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba" or rec["ours"]["empty"] != "raised":
        raise SystemExit("stemlock nihon show identity failed")
    if rec["ours"]["empty_log"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("stemlock nihon log identity failed")
