...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem.lay")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    stem = Path("/tmp/aletheia-persyst-official-empty-stem")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-stem.lay")
    p.write_bytes(b"")
    try:
        read_persyst(p)
        empty = "accepted"
    ...
    try:
        write_persyst(Path("/tmp/aletheia-empty-persyst-comments-stem"), [1], comments=[])
        empty_comments = "accepted"
    except ValueError:
        empty_comments = "raised"
    rec = {
        "schema": "stemlock.show_persyst.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Persyst aba and go/end; suffix links 3; empty Persyst is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba" or rec["ours"]["empty"] != "raised":
        raise SystemExit("stemlock persyst show identity failed")
    if rec["ours"]["n"] != 3:
        raise SystemExit("stemlock persyst n identity failed")
    if rec["ours"]["empty_comments"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("stemlock persyst comment identity failed")
