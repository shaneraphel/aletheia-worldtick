...
"""Show: MNE and edfio on a BDF tape (an empty recording)."""
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-stem.bdf")
    p.write_bytes(b"")
    try:
...
def main():
    word = "".join(chr(int(v)) for v in read_bdf(BDF)["samples"])
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-stem.bdf")
    p.write_bytes(b"")
    try:
        read_bdf(p)
        empty = "accepted"
    ...
    empty_annot = "raised"
    try:
        write_bdf(Path("/tmp/aletheia-empty-bdf-annot-stem.bdf"), [1], annotations=[])
        empty_annot = "accepted"
    except ValueError:
        empty_annot = "raised"
    rec = {
        "schema": "stemlock.show_bdf.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE and edfio read BDF aba and go/end; suffix links 3; empty BDF is absence",
        "theirs": theirs(),
        "ours": {
            "n": bdf_occupancy(BDF),
            "word": word,
    ...
    if rec["ours"]["links"] != 3 or rec["ours"]["word"] != "aba" or rec["ours"]["empty"] != "raised":
        raise SystemExit("stemlock bdf show identity failed")
    if rec["ours"]["empty_annot"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("stemlock bdf annot identity failed")
    if rec["theirs"]["n_times"] != 3 or rec["theirs"]["empty_bdf"] != "raised":
