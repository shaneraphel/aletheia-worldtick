...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-gap.EEG")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    nolog = Path("/tmp/aletheia-nolog-nihon-gap")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-gap.EEG")
    p.write_bytes(b"")
    try:
        read_nihon(p)
        empty = "accepted"
    ...
    empty_log = "raised"
    try:
        write_nihon(Path("/tmp/aletheia-empty-nihon-log-gap"), [1], events=[])
        empty_log = "accepted"
    except ValueError:
        empty_log = "raised"
    rec = {
        "schema": "gaplock.show_nihon.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Nihon Kohden onsets [1,3,2] and go/end; empty-slot day 2; empty Nihon Kohden is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["day"] != 2 or rec["ours"]["empty"] != "raised":
        raise SystemExit("gaplock nihon show identity failed")
    if rec["ours"]["empty_log"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("gaplock nihon log identity failed")
