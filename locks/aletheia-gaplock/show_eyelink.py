...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-gap.asc")
    p.write_text("")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-gap.asc")
    p.write_text("")
    try:
        read_eyelink(p)
        empty = "accepted"
    ...
    empty_msg = "raised"
    try:
        write_eyelink(Path("/tmp/aletheia-empty-eyelink-msg-gap.asc"), [1], messages=[])
        empty_msg = "accepted"
    except ValueError:
        empty_msg = "raised"
    rec = {
        "schema": "gaplock.show_eyelink.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads EyeLink onsets [1,3,2] and go/end; empty-slot day 2; empty EyeLink is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["day"] != 2 or rec["ours"]["empty"] != "raised":
        raise SystemExit("gaplock eyelink show identity failed")
    if rec["ours"]["empty_msg"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("gaplock eyelink msg identity failed")
