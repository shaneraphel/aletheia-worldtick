...
def main():
    ...
    try:
        ...
        empty_evt = "raised"
    rec = {
        "schema": "gaplock.show_nirx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads NIRx onsets [1,3,2] on S1_D1 and stim 1.0/2.0 at 1.0/2.0 s; empty NIRx is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["n"] != N or rec["ours"]["empty"] != "raised":
        raise SystemExit("gaplock nirx show identity failed")
    if rec["ours"]["empty_evt"] != "raised" or rec["ours"]["annot"] != ["1.0", "2.0"]:
        raise SystemExit("gaplock nirx evt identity failed")
