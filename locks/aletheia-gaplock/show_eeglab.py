...
def main():
    bulbs = read_eeglab(SET)
    empty = "raised"
    p = Path("/tmp/aletheia-empty-gap.set")
    ...
    empty_events = "raised"
    try:
        write_eeglab(Path("/tmp/aletheia-empty-events-gap.set"), [1], events=[])
        empty_events = "accepted"
    except ValueError:
        empty_events = "raised"
    rec = {
        "schema": "gaplock.show_eeglab.v1",
        "used": "https://eeglab.org/",
        "built": "EEGLAB onsets [1,3,2] give empty-slot day 2; EEG.event go/end; empty set is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["day"] != 2 or rec["ours"]["empty"] != "raised":
        raise SystemExit("gaplock eeglab show identity failed")
    if rec["ours"]["empty_events"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("gaplock eeglab event identity failed")
