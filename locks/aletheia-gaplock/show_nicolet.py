...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-gap.data")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-gap.data")
    p.write_bytes(b"")
    try:
        read_nicolet(p)
        empty = "accepted"
    ...
    try:
        write_nicolet(Path("/tmp/aletheia-empty-nicolet-samples-gap"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "gaplock.show_nicolet.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Nicolet onsets [1,3,2]; empty-slot day 2; empty Nicolet is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["day"] != 2 or rec["ours"]["empty"] != "raised":
        raise SystemExit("gaplock nicolet show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("gaplock nicolet empty identity failed")
