...
def theirs() -> dict:
    ...
    empty_annot = Path("/tmp/aletheia-empty-gap-annot.txt")
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-gap-mne.gdf")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty_annot = "raised"
    qa = Path("/tmp/aletheia-empty-ours-gap-annot.txt")
    qa.write_bytes(b"")
    try:
        ...
        empty_annot = "raised"
    rec = {
        "schema": "gaplock.show_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE annotations copy BIDS trial_type go/end; empty trial_type is absence; MNE reads GDF onsets [1,3,2]; empty-slot day 2; empty GDF is absence",
        "theirs": theirs(),
        "ours": {"annot": read_mne_annot_descriptions(ANNOT), "empty_annot": empty_annot, "edf_annot": read_edf_annotations(EDF), "samples": bulbs, "day": k_empty_slots(bulbs, 1)},
    ...
    if rec["ours"]["day"] != 2:
        raise SystemExit("gaplock mne show identity failed")
    if rec["theirs"]["gdf_n_times"] != 3 or rec["theirs"]["empty_gdf"] != "raised":
        raise SystemExit("mne gap identity failed")
