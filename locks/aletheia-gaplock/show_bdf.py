...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-gap.bdf")
    p.write_bytes(b"")
    try:
...
def main():
    bulbs = read_bdf(BDF)["samples"]
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-gap.bdf")
    p.write_bytes(b"")
    try:
        read_bdf(p)
        empty = "accepted"
    ...
    empty_annot = "raised"
    try:
        write_bdf(Path("/tmp/aletheia-empty-bdf-annot-gap.bdf"), [1], annotations=[])
        empty_annot = "accepted"
    except ValueError:
        empty_annot = "raised"
    rec = {
        "schema": "gaplock.show_bdf.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE and edfio read BDF onsets [1,3,2] and go/end; empty-slot day 2; empty BDF is absence",
        "theirs": theirs(),
        "ours": {
            "n": bdf_occupancy(BDF),
            "samples": bulbs,
    ...
    if rec["ours"]["day"] != 2 or rec["ours"]["empty"] != "raised":
        raise SystemExit("gaplock bdf show identity failed")
    if rec["ours"]["empty_annot"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("gaplock bdf annot identity failed")
