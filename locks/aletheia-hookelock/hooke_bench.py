...
def main():
    ...
    for trial in range(WARMUP + N_TRIALS):
        t0 = time.perf_counter(); acc = hooke_law(steps); at = time.perf_counter()-t0
        t1 = time.perf_counter(); nv = naive(steps); bt = time.perf_counter()-t1
        if acc != nv:
            raise SystemExit("hooke and naive disagree")
        if trial < WARMUP:
            continue
    ...
    rec = {
        "schema": "hookelock.hooke_bench.v1", "seed": SEED, "n": N,
        "n_trials": N_TRIALS, "warmup": WARMUP, "n_paired": len(a),
        "hooke_seconds_median": statistics.median(a),
        "naive_seconds_median": statistics.median(b),
        "hooke_seconds": a, "naive_seconds": b,
