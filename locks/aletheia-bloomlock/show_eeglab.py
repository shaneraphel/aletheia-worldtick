...
def main():
    keys = read_eeglab(SET)
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.set")
    ...
    empty_events = "raised"
    try:
        write_eeglab(Path("/tmp/aletheia-empty-events-bloom.set"), [1], events=[])
        empty_events = "accepted"
    except ValueError:
        empty_events = "raised"
    rec = {
        "schema": "bloomlock.show_eeglab.v1",
        "used": "https://eeglab.org/",
        "built": "EEGLAB onsets [1,2,3] maybe-membership 1; EEG.event go/end; empty set is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock eeglab show identity failed")
    if rec["ours"]["empty_events"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("bloomlock eeglab event identity failed")
