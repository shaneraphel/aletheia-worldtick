...
"""Show: I used MNE on a Nihon Kohden tape, and I refused an empty recording."""
...
EEG = Path(__file__).resolve().parent / "resources" / "synthetic" / "tape_nihon.EEG"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.EEG")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    nolog = Path("/tmp/aletheia-nolog-nihon-bloom")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-bloom.EEG")
    p.write_bytes(b"")
    try:
        read_nihon(p)
        empty = "accepted"
    ...
    empty_log = "raised"
    try:
        write_nihon(Path("/tmp/aletheia-empty-nihon-log-bloom"), [1], events=[])
        empty_log = "accepted"
    except ValueError:
        empty_log = "raised"
    rec = {
        "schema": "bloomlock.show_nihon.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Nihon Kohden onsets [1,2,3] and go/end; maybe-membership 1; empty Nihon Kohden is absence",
        "theirs": theirs(),
        "ours": {
            "n": nihon_occupancy(EEG),
            "samples": nums,
            "maybe": bloom_maybe(nums, 16, 2, 2),
            "empty": empty,
            "empty_log": empty_log,
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock nihon show identity failed")
    if rec["ours"]["empty_log"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("bloomlock nihon log identity failed")
    if rec["theirs"]["n_times"] != 3 or rec["theirs"]["empty_eeg"] != "raised":
