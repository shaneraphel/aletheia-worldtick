...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.nxe")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-bloom.nxe")
    p.write_bytes(b"")
    try:
        read_eximia(p)
        empty = "accepted"
    ...
    try:
        write_eximia(Path("/tmp/aletheia-empty-eximia-samples-bloom.nxe"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "bloomlock.show_eximia.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Eximia onsets [1,2,3] on Cz; maybe-membership 1; empty Eximia is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock eximia show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("bloomlock eximia empty identity failed")
