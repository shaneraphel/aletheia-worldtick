...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.ns3")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-bloom.ns3")
    p.write_bytes(b"")
    try:
        read_nsx(p)
        empty = "accepted"
    ...
    try:
        write_nsx(Path("/tmp/aletheia-empty-nsx-samples-bloom.ns3"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "bloomlock.show_nsx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads NSX onsets [1,2,3]; maybe-membership 1; empty NSX is absence",
        "theirs": theirs(),
        "ours": {
            "n": nsx_occupancy(NSX),
            "samples": nums,
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock nsx show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("bloomlock nsx empty identity failed")
