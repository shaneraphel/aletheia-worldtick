...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.cnt")
    p.write_bytes(b"")
    try:
...
def main():
    keys = read_cnt(CNT)["samples"]
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-bloom.cnt")
    p.write_bytes(b"")
    try:
        read_cnt(p)
        empty = "accepted"
    ...
    empty_events = "raised"
    try:
        write_cnt(Path("/tmp/aletheia-empty-cnt-events-bloom.cnt"), [1], events=[])
        empty_events = "accepted"
    except ValueError:
        empty_events = "raised"
    rec = {
        "schema": "bloomlock.show_cnt.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads CNT onsets [1,2,3] and stim 1/2; maybe-membership 1; empty CNT is absence",
        "theirs": theirs(),
        "ours": {
            "n": cnt_occupancy(CNT),
            "samples": keys,
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock cnt show identity failed")
    if rec["ours"]["empty_events"] != "raised" or rec["ours"]["annot"] != ["1", "2"]:
        raise SystemExit("bloomlock cnt event identity failed")
