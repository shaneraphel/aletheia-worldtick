...
DATA = Path(__file__).resolve().parent / "resources" / "synthetic" / "tape_nicolet.data"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.data")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-bloom.data")
    p.write_bytes(b"")
    try:
        read_nicolet(p)
        empty = "accepted"
    ...
    try:
        write_nicolet(Path("/tmp/aletheia-empty-nicolet-samples-bloom"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "bloomlock.show_nicolet.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Nicolet onsets [1,2,3]; maybe-membership 1; empty Nicolet is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock nicolet show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("bloomlock nicolet empty identity failed")
    if rec["theirs"]["n_times"] != 3 or rec["theirs"]["empty_nicolet"] != "raised":
