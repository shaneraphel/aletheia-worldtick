...
def main():
    ...
    try:
        ...
        empty_evt = "raised"
    rec = {
        "schema": "bloomlock.show_nirx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads NIRx onsets [1,2,3] on S1_D1 and stim 1.0/2.0 at 1.0/2.0 s; empty NIRx is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["n"] != N or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock nirx show identity failed")
    if rec["ours"]["empty_evt"] != "raised" or rec["ours"]["annot"] != ["1.0", "2.0"]:
        raise SystemExit("bloomlock nirx evt identity failed")
