...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.vhdr")
    p.write_text("")
    try:
...
def main():
    keys = read_brainvision(VHDR)["samples"]
    rec = {
        "schema": "bloomlock.show_brainvision_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "MNE reads BrainVision [1,2,3]; maybe-membership 1; empty .vhdr is absence",
        "theirs": theirs(),
        "ours": {"annot": read_brainvision_annotations(VHDR), "samples": keys, "maybe": bloom_maybe(keys, 16, 2, 2)},
    ...
    if rec["ours"]["maybe"] != 1:
        raise SystemExit("bloomlock brainvision show identity failed")
    if rec["theirs"]["n_times"] != 3 or rec["theirs"]["empty_vhdr"] != "raised":
        raise SystemExit("mne bloom brainvision identity failed")
