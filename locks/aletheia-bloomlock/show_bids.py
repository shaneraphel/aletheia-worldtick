...
def main():
    keys = read_bids_onsets(EV)
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom-events.tsv")
    ...
    try:
        ...
        empty = "raised"
    rec = {
        "schema": "bloomlock.show_bids.v1",
        "used": "https://bids.neuroimaging.io/",
        "built": "BIDS events [1,2,3] maybe-membership 1; empty events are absence",
        "ours": {"n": bids_eeg_occupancy(CH, EV), "maybe": bloom_maybe(keys, 16, 2, 2), "markers": read_bids_trial_types(MK), "annot": read_mne_annot_descriptions(ANNOT), "empty": empty},
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock bids show identity failed")
    if rec["ours"]["markers"] != ["go", "end"] or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("bloomlock bids marker identity failed")
