...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.lay")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    stem = Path("/tmp/aletheia-persyst-official-empty-bloom")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-bloom.lay")
    p.write_bytes(b"")
    try:
        read_persyst(p)
        empty = "accepted"
    ...
    try:
        write_persyst(Path("/tmp/aletheia-empty-persyst-comments-bloom"), [1], comments=[])
        empty_comments = "accepted"
    except ValueError:
        empty_comments = "raised"
    rec = {
        "schema": "bloomlock.show_persyst.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Persyst onsets [1,2,3] and go/end; maybe-membership 1; empty Persyst is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock persyst show identity failed")
    if rec["ours"]["n"] != 3:
        raise SystemExit("bloomlock persyst n identity failed")
    if rec["ours"]["empty_comments"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("bloomlock persyst comment identity failed")
