...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom.bdf")
    p.write_bytes(b"")
    try:
...
def main():
    keys = read_bdf(BDF)["samples"]
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-bloom.bdf")
    p.write_bytes(b"")
    try:
        read_bdf(p)
        empty = "accepted"
    ...
    empty_annot = "raised"
    try:
        write_bdf(Path("/tmp/aletheia-empty-bdf-annot-bloom.bdf"), [1], annotations=[])
        empty_annot = "accepted"
    except ValueError:
        empty_annot = "raised"
    rec = {
        "schema": "bloomlock.show_bdf.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE and edfio read BDF onsets [1,2,3] and go/end; maybe-membership 1; empty BDF is absence",
        "theirs": theirs(),
        "ours": {
            "n": bdf_occupancy(BDF),
            "samples": keys,
    ...
    if rec["ours"]["maybe"] != 1 or rec["ours"]["empty"] != "raised":
        raise SystemExit("bloomlock bdf show identity failed")
    if rec["ours"]["empty_annot"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("bloomlock bdf annot identity failed")
