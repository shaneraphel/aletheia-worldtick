...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-bloom-edfio.edf")
    p.write_bytes(b"")
    try:
        edfio.read_edf(p)
        empty = "accepted"
...
def main():
    keys = read_edf(EDF)["samples"]
    rec = {
        "schema": "bloomlock.show_edfio.v1",
        "used": "https://github.com/the-siesta-group/edfio",
        "built": "edfio reads EDF onsets [1,2,3]; maybe-membership 1; empty EDF is absence",
        "theirs": theirs(),
        "ours": {"samples": keys, "maybe": bloom_maybe(keys, 16, 2, 2), "edf_annot": read_edf_annotations(EDF)},
    ...
    if rec["ours"]["maybe"] != 1:
        raise SystemExit("bloomlock edfio show identity failed")
    if rec["theirs"]["n_samples"] != 3 or rec["theirs"]["empty"] != "raised":
        raise SystemExit("edfio bloom identity failed")
