...
"""Show: I used MNE on a GDF 1.25 tape, and I refused an empty sick set."""
...
from gdfocc import read_gdf_sick
...
def theirs() -> dict:
    ...
    empty_annot = Path("/tmp/aletheia-empty-sick-annot.txt")
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick-mne.gdf")
    p.write_bytes(b"")
    try:
    ...
    return {
        ...
        "gdf_n_channels": int(len(gdf.ch_names)),
        "empty_gdf": empty,
    }
...
def main():
    ...
    empty_annot = "raised"
    qa = Path("/tmp/aletheia-empty-ours-sick-annot.txt")
    qa.write_bytes(b"")
    try:
        ...
        empty_annot = "raised"
    rec = {
        "schema": "sicklock.show_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE annotations copy BIDS trial_type go/end; empty trial_type is absence; MNE reads five GDF channels, sick 0 and 4; four orders; empty GDF is absence",
        "theirs": theirs(),
        "ours": {"annot": read_mne_annot_descriptions(ANNOT), "empty_annot": empty_annot, "edf_annot": read_edf_annotations(EDF), "n": n, "sick": sick, "sequences": infection_sequences(n, sick)},
    ...
    if rec["ours"]["sequences"] != 4:
        raise SystemExit("sicklock mne show identity failed")
    if rec["theirs"]["gdf_n_channels"] != 5 or rec["theirs"]["empty_gdf"] != "raised":
        raise SystemExit("mne sick identity failed")
