...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick.EEG")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    nolog = Path("/tmp/aletheia-nolog-nihon-sick")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-sick.EEG")
    p.write_bytes(b"")
    try:
        read_nihon(p)
        empty = "accepted"
    ...
    empty_log = "raised"
    try:
        write_nihon(Path("/tmp/aletheia-empty-nihon-log-sick"), [1], events=[])
        empty_log = "accepted"
    except ValueError:
        empty_log = "raised"
    rec = {
        "schema": "sicklock.show_nihon.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads five Nihon Kohden samples, sick 0 and 4, and go/end; four orders; empty Nihon Kohden is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["sequences"] != 4 or rec["ours"]["empty"] != "raised":
        raise SystemExit("sicklock nihon show identity failed")
    if rec["ours"]["empty_log"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("sicklock nihon log identity failed")
