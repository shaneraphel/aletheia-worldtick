...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick.asc")
    p.write_text("")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-sick.asc")
    p.write_text("")
    try:
        read_eyelink(p)
        empty = "accepted"
    ...
    empty_msg = "raised"
    try:
        write_eyelink(Path("/tmp/aletheia-empty-eyelink-msg-sick.asc"), [1], messages=[])
        empty_msg = "accepted"
    except ValueError:
        empty_msg = "raised"
    rec = {
        "schema": "sicklock.show_eyelink.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads five EyeLink samples, sick 0 and 4, and go/end; four orders; empty EyeLink is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["sequences"] != 4 or rec["ours"]["empty"] != "raised":
        raise SystemExit("sicklock eyelink show identity failed")
    if rec["ours"]["empty_msg"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("sicklock eyelink msg identity failed")
