...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick.ns3")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-sick.ns3")
    p.write_bytes(b"")
    try:
        read_nsx(p)
        empty = "accepted"
    ...
    try:
        write_nsx(Path("/tmp/aletheia-empty-nsx-samples-sick.ns3"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "sicklock.show_nsx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads five NSX samples, sick 0 and 4; four orders; empty NSX is absence",
        "theirs": theirs(),
        "ours": {
            "n": nsx_occupancy(NSX),
            "samples": nums,
    ...
    if rec["ours"]["sequences"] != 4 or rec["ours"]["empty"] != "raised":
        raise SystemExit("sicklock nsx show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("sicklock nsx empty identity failed")
