...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick.lay")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    stem = Path("/tmp/aletheia-persyst-official-empty-sick")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-sick.lay")
    p.write_bytes(b"")
    try:
        read_persyst(p)
        empty = "accepted"
    ...
    try:
        write_persyst(Path("/tmp/aletheia-empty-persyst-comments-sick"), [1], comments=[])
        empty_comments = "accepted"
    except ValueError:
        empty_comments = "raised"
    rec = {
        "schema": "sicklock.show_persyst.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads five Persyst samples, sick 0 and 4, and go/end; four orders; empty Persyst is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["sequences"] != 4 or rec["ours"]["empty"] != "raised":
        raise SystemExit("sicklock persyst show identity failed")
    if rec["ours"]["n"] != 5:
        raise SystemExit("sicklock persyst n identity failed")
    if rec["ours"]["empty_comments"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("sicklock persyst comment identity failed")
