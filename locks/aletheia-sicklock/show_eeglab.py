...
"""Show: I used an EEGLAB set, and I refused an empty sick set."""
from __future__ import annotations
import json, platform, sys
from pathlib import Path
from eeglabocc import read_eeglab, read_eeglab_events, read_eeglab_sick, write_eeglab
...
def main():
    n, sick = read_eeglab_sick(SET)
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick.set")
    ...
    empty_events = "raised"
    try:
        write_eeglab(Path("/tmp/aletheia-empty-events-sick.set"), [1], events=[])
        empty_events = "accepted"
    except ValueError:
        empty_events = "raised"
    rec = {
        "schema": "sicklock.show_eeglab.v1",
        "used": "https://eeglab.org/",
        "built": "five EEGLAB channels, sick 0 and 4, four orders; EEG.event go/end; empty set is absence",
        "theirs": theirs(),
        "ours": {
            "n": n,
            "sick": sick,
    ...
    if rec["ours"]["sequences"] != 4 or rec["ours"]["empty"] != "raised":
        raise SystemExit("sicklock eeglab show identity failed")
    if rec["ours"]["empty_events"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("sicklock eeglab event identity failed")
