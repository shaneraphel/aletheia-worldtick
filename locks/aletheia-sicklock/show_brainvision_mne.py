...
"""Show: I used MNE on a BrainVision tape, and I refused an empty sick set."""
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick.vhdr")
    p.write_text("")
    try:
...
def main():
    ...
    rec = {
        "schema": "sicklock.show_brainvision_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "MNE reads five BrainVision samples, sick 0 and 4; four orders; empty .vhdr is absence",
        "theirs": theirs(),
        "ours": {"annot": read_brainvision_annotations(VHDR), "n": n, "sick": sick, "sequences": infection_sequences(n, sick)},
    ...
    if rec["ours"]["sequences"] != 4:
        raise SystemExit("sicklock brainvision show identity failed")
    if rec["theirs"]["n_times"] != 5 or rec["theirs"]["empty_vhdr"] != "raised":
        raise SystemExit("mne sick brainvision identity failed")
