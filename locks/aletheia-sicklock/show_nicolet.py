...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-sick.data")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-sick.data")
    p.write_bytes(b"")
    try:
        read_nicolet(p)
        empty = "accepted"
    ...
    try:
        write_nicolet(Path("/tmp/aletheia-empty-nicolet-samples-sick"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "sicklock.show_nicolet.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads five Nicolet samples, sick 0 and 4; four orders; empty Nicolet is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["sequences"] != 4 or rec["ours"]["empty"] != "raised":
        raise SystemExit("sicklock nicolet show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("sicklock nicolet empty identity failed")
