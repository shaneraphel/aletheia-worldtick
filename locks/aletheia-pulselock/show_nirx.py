...
NIRX = Path(__file__).resolve().parent / "resources" / "synthetic" / "pulse_nirx"
LOCK = "pulselock"
N = 5
...
def main():
    ...
    try:
        ...
        empty_evt = "raised"
    rec = {
        "schema": "pulselock.show_nirx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads NIRx [1,2,3,-1,2] on S1_D1 and stim 1.0/2.0 at 1.0/2.0 s; empty NIRx is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["n"] != N or rec["ours"]["empty"] != "raised":
        raise SystemExit("pulselock nirx show identity failed")
    if rec["ours"]["empty_evt"] != "raised" or rec["ours"]["annot"] != ["1.0", "2.0"]:
        raise SystemExit("pulselock nirx evt identity failed")
