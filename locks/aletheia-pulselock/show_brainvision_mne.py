...
"""Show: I used MNE on a BrainVision pulse tape, and I refused an empty header."""
...
VHDR = Path(__file__).resolve().parent / "resources" / "synthetic" / "pulse.vhdr"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-pulse.vhdr")
    p.write_text("")
    try:
...
def main():
    nums = read_brainvision(VHDR)["samples"]
    rec = {
        "schema": "pulselock.show_brainvision_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "MNE reads BrainVision [1,2,3,-1,2]; strength 22; empty .vhdr is absence",
        "theirs": theirs(),
        "ours": {"annot": read_brainvision_annotations(VHDR), "samples": nums, "strength": max_k_subarray_strength(nums, 3)},
    ...
    if rec["ours"]["strength"] != 22:
        raise SystemExit("pulselock brainvision show identity failed")
    if rec["theirs"]["n_times"] != 5 or rec["theirs"]["empty_vhdr"] != "raised":
        raise SystemExit("mne pulse brainvision identity failed")
