...
EDF = ROOT / "tape_eeg.edf"
GDF = ROOT / "pulse.gdf"
ANNOT = ROOT / "tape_annotations.txt"
...
def theirs() -> dict:
    ...
    empty_annot = Path("/tmp/aletheia-empty-pulse-annot.txt")
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-pulse-mne.edf")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty_annot = "raised"
    qa = Path("/tmp/aletheia-empty-ours-pulse-annot.txt")
    qa.write_bytes(b"")
    try:
        ...
        empty_annot = "raised"
    rec = {
        "schema": "pulselock.show_mne.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE annotations copy BIDS trial_type go/end; empty trial_type is absence; MNE reads the pulse EDF and GDF; strength 22; empty EDF is absence",
        "theirs": theirs(),
        "ours": {"annot": read_mne_annot_descriptions(ANNOT), "empty_annot": empty_annot, "edf_annot": read_edf_annotations(EDF), "samples": nums, "strength": max_k_subarray_strength(nums, 3)},
    ...
    if rec["ours"]["strength"] != 22:
        raise SystemExit("pulselock mne show identity failed")
    if rec["theirs"]["gdf_n_times"] != 5 or rec["theirs"]["empty_edf"] != "raised":
        raise SystemExit("mne pulse identity failed")
