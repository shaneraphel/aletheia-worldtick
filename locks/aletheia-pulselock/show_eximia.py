...
NXE = Path(__file__).resolve().parent / "resources" / "synthetic" / "pulse.nxe"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-pulse.nxe")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-pulse.nxe")
    p.write_bytes(b"")
    try:
        read_eximia(p)
        empty = "accepted"
    ...
    try:
        write_eximia(Path("/tmp/aletheia-empty-eximia-samples-pulse.nxe"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "pulselock.show_eximia.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Eximia [1,2,3,-1,2] on Cz; strength 22; empty Eximia is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["strength"] != 22 or rec["ours"]["empty"] != "raised":
        raise SystemExit("pulselock eximia show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("pulselock eximia empty identity failed")
