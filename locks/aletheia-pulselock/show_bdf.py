...
BDF = Path(__file__).resolve().parent / "resources" / "synthetic" / "tape_eeg.bdf"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-pulse.bdf")
    p.write_bytes(b"")
    try:
...
def main():
    nums = read_bdf(BDF)["samples"]
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-pulse.bdf")
    p.write_bytes(b"")
    try:
        read_bdf(p)
        empty = "accepted"
    ...
    empty_annot = "raised"
    try:
        write_bdf(Path("/tmp/aletheia-empty-bdf-annot-pulse.bdf"), [1], annotations=[])
        empty_annot = "accepted"
    except ValueError:
        empty_annot = "raised"
    rec = {
        "schema": "pulselock.show_bdf.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE and edfio read BDF [1,2,3,-1,2] and go/end; strength 22; empty BDF is absence",
        "theirs": theirs(),
        "ours": {
            "n": bdf_occupancy(BDF),
            "samples": nums,
    ...
    if rec["ours"]["strength"] != 22 or rec["ours"]["empty"] != "raised":
        raise SystemExit("pulselock bdf show identity failed")
    if rec["ours"]["empty_annot"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("pulselock bdf annot identity failed")
