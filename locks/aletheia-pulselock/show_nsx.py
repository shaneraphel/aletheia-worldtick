...
NSX = Path(__file__).resolve().parent / "resources" / "synthetic" / "pulse.ns3"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-pulse.ns3")
    p.write_bytes(b"")
    try:
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-pulse.ns3")
    p.write_bytes(b"")
    try:
        read_nsx(p)
        empty = "accepted"
    ...
    try:
        write_nsx(Path("/tmp/aletheia-empty-nsx-samples-pulse.ns3"), [])
        empty_samples = "accepted"
    except ValueError:
        empty_samples = "raised"
    rec = {
        "schema": "pulselock.show_nsx.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads NSX [1,2,3,-1,2]; strength 22; empty NSX is absence",
        "theirs": theirs(),
        "ours": {
            "n": nsx_occupancy(NSX),
            "samples": nums,
    ...
    if rec["ours"]["strength"] != 22 or rec["ours"]["empty"] != "raised":
        raise SystemExit("pulselock nsx show identity failed")
    if rec["ours"]["empty_samples"] != "raised":
        raise SystemExit("pulselock nsx empty identity failed")
    if rec["theirs"]["n_times"] != 5 or rec["theirs"]["empty_nsx"] != "raised":
