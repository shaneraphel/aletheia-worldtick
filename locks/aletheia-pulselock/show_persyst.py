...
LAY = Path(__file__).resolve().parent / "resources" / "synthetic" / "pulse_persyst.lay"
...
def theirs() -> dict:
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-pulse.lay")
    p.write_bytes(b"")
    try:
        ...
        empty = "raised"
    stem = Path("/tmp/aletheia-persyst-official-empty-pulse")
...
def main():
    ...
    empty = "raised"
    p = Path("/tmp/aletheia-empty-ours-pulse.lay")
    p.write_bytes(b"")
    try:
        read_persyst(p)
        empty = "accepted"
    ...
    try:
        write_persyst(Path("/tmp/aletheia-empty-persyst-comments-pulse"), [1], comments=[])
        empty_comments = "accepted"
    except ValueError:
        empty_comments = "raised"
    rec = {
        "schema": "pulselock.show_persyst.v1",
        "used": "https://github.com/mne-tools/mne-python",
        "built": "official MNE reads Persyst [1,2,3,-1,2] and go/end; strength 22; empty Persyst is absence",
        "theirs": theirs(),
        "ours": {
    ...
    if rec["ours"]["strength"] != 22 or rec["ours"]["empty"] != "raised":
        raise SystemExit("pulselock persyst show identity failed")
    if rec["ours"]["n"] != 5:
        raise SystemExit("pulselock persyst n identity failed")
    if rec["ours"]["empty_comments"] != "raised" or rec["ours"]["annot"] != ["go", "end"]:
        raise SystemExit("pulselock persyst comment identity failed")
    if rec["theirs"]["n_times"] != 5 or rec["theirs"]["empty_lay"] != "raised":
