...
def main():
    ...
    for trial in range(WARMUP + N_TRIALS):
        t0 = time.perf_counter(); acc = jarvis_hull(pts); at = time.perf_counter()-t0
        t1 = time.perf_counter(); nv = graham(pts); bt = time.perf_counter()-t1
        if acc != nv:
            raise SystemExit("jarvis and graham disagree")
        if trial < WARMUP:
            continue
    ...
    rec = {
        "schema": "marchlock.jarvis_bench.v1", "seed": SEED, "n": N,
        "n_trials": N_TRIALS, "warmup": WARMUP, "n_paired": len(a),
        "jarvis_seconds_median": statistics.median(a),
        "graham_seconds_median": statistics.median(b),
        "jarvis_seconds": a, "graham_seconds": b,
        "hull_first": first, "hull_second": second,
        "hull_identical": first == second, "hull_equals_graham": True,
